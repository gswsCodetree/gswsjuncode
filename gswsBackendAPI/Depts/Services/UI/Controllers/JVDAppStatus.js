(function () {
    ...
    app.controller("JVDAppStatus", ["$scope", "$state", "$log", "Ser_Services", Arg_CTRL]);

    function Arg_CTRL(scope, state, log, tai_services) {
        scope.pagename = "JVDAppStatus";
        ...
        scope.GetStatus = function () {
            ...
            //}
                ...
                tai_services.POSTENCRYPTAPI("GetJVDAppStatus", req, token, function (value) {

                    if (value.data.Status == 100) {
                        console.log(value.data.Details);
                        var data = value.data.Details;
                        if (!data.errorMsg) {
                            scope.stu_name = data.stu_name;
                            scope.gender = data.gender;
                            scope.coll_name = data.coll_name;
                            scope.course_year = data.course_year;
                            scope.sch_type_name = data.sch_type_name;
                            scope.status_desc = data.status_desc;
