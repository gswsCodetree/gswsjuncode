(function () {
    ...
    app.controller("PTRegistrationControllers", ["$scope", "$state", "$log", "CT_Services", PTREG_CTRL]);
    ...
    function PTREG_CTRL(scope, state, log, ptreg_services) {
        ...
        // Get Details By TIN
        scope.GetDetails = function (tno) {
            if (tno) {
                var obj = {
                    "action": "GETREG",
                    "tin": tno
                }
                ptreg_services.POSTENCRYPTAPI("GetDetailsByTIN", obj, token, function (value) {
        ...
        scope.Register = function () {
            ...
            if (Step1Validations() && Step2Validations() && scope.BanksTable.length > 0 && scope.UplFilesTable.length > 1) {
                ...
                for (i = 0; i < scope.PartnersTable.length; i++) {
                    ...
                    ParObj.DOB = scope.PartnersTable[i].DOB;
                ...
                ptreg_services.POSTENCRYPTAPI("SubmitData", obj, token, function (value) {

                    if (value.data.Status == 100) {
                        if (value.data.Details.status_cd == "1") {
                            scope.IsSuccess = true;
                            swal("Success", "Profession Tax Registered Successfully. \n RNR No : " + value.data.Details.RNR, "success");
                        }
                        else {
                            var Message = "";
        ...
        scope.Home = function () { window.location.reload(); }
        ...
        function FillDetails(data) {
            ...
            //        bObj.bankname = baobj[i].bankname;
